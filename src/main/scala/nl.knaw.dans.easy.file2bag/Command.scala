...
//      case None => // handle command line without subcommands
  ...
  private def runAsService(app: EasyAddFilesToBagApp): Try[FeedBackMessage] = Try {
    val service = new EasyAddFilesToBagService(configuration.serverPort, Map(
      "/" -> new EasyAddFilesToBagServlet(app, configuration.version),
    ))
